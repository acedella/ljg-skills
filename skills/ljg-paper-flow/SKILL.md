---
name: ljg-paper-flow
description: "Paper workflow: read papers + cast viewfinder (取景框) library cards in one go. Takes one or more arxiv links, paper URLs, PDFs, or paper names. For each paper, runs ljg-paper (generates a Markdown analysis note) then ljg-library (distills the paper's viewfinder (取景框) into a 2050 library card PNG). Use when user says '论文流' (paper flow), 'paper flow', '读论文并做卡片' (read papers and make cards), '论文卡片' (paper card), or provides multiple papers wanting both analysis and cards."
user_invocable: true
version: "1.1.0"
---
...
Invoke the Skill tool to run `ljg-paper`, passing in the paper's source. Wait for completion to get the generated Markdown note path.

**Step B — Cast the Viewfinder Card (ljg-library):**

Read the Markdown note generated in Step A — it has already fully broken down this paper's proposition, its core, and the new way of seeing the world it brings, which is exactly the "fully-thought-through viewfinder idea" that ljg-library needs. Hand this idea, together with the paper's title / authors / arxiv info, to the Skill tool to run `ljg-library`: take its flexible-input path (when an idea is already given, it only validates + draws, skipping distillation from scratch), producing a viewfinder library card PNG. Wait for completion to get the PNG path.
...
- The card's viewfinder comes from the generated Markdown note (the fully broken-down proposition + core), not the original paper
